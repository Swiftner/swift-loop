...
**Phase 2 — PreviewAdapter; delete `render-loop.ts`.** Implement `PreviewAdapter` against SVG/DOM. Rewire `src/preview/host.ts` so each loop instance owns a `PreviewAdapter` and goes through the shared orchestrator instead of `renderLoop()`. Delete `src/preview/render-loop.ts`. Run the contract test suite against `PreviewAdapter` in JSDOM. Manually verify the playground still works (zoom, drag, rotate handles, multi-loop, undo, export). **Also independently shippable** — and at this point any rough edge in the `HostAdapter` interface has surfaced. Refine the interface before Phase 3.

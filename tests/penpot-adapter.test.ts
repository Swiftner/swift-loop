...
describe('PenpotAdapter', () => {
  ...
  it('applies transform with a relative rotation delta around the center', async () => {
    const s = new FakeShape('a')
    s.rotation = 10
    ...
    expect(s.x).toBe(100)
    expect(s.y).toBe(200)
    // delta = 45 - 10 = 35, center = (100+20, 200+15)
    expect(s.rotateLog).toEqual([{ angle: 35, center: { x: 120, y: 215 } }])
  })
